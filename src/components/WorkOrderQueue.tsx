...
export function WorkOrderQueue({ orders, selectedId, onSelect }: Props) {
  return (
    <section className="queue-panel glass">
      <div className="section-head">
        <div><h2>Work Order Queue</h2><span>{orders.length} results</span></div>
      ...
      <div className="order-list">
        {orders.map(order => {
          ...
          return (
            <button
              key={order.ticket_id}
              className={`order-row glass-inset ${selectedId === order.ticket_id ? 'selected' : ''}`}
            ...
            >
              ...
              <div className="order-id"><strong>{order.ticket_id}</strong><span>{ago(order.timestamp)}</span></div>
              ...
              <div className="assignee">
                ...
                <div><strong>{order.technician || 'Unassigned'}</strong><span>{order.technician ? 'Technician' : 'Needs assignment'}</span></div>
              </div>
